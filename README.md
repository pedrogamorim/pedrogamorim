## Hi there!👋 Eu sou o Pedro Gabriel!

- 🔭 I’m currently working administration
- 🌱 I’m currently learning Spring Boot
- 📫 How reach me: amorimjeronimo.gabriel@outlook.com
- 😄 Pronouns: ele/dele


<!-- Painel de Tecnologias Usadas -->

<div style="background-color: #1e1e1e; border: 1px solid #333; border-radius: 10px; padding: 20px; width: 100%; max-width: 500px; color: white; font-family: sans-serif;">

  <h3 style="margin-bottom: 20px;">Tecnologias</h3>

  <!-- Barra de linguagens -->
  <div style="height: 10px; width: 100%; background-color: #333; border-radius: 5px; overflow: hidden; margin-bottom: 15px;">
    <div style="width: 50%; background-color: orange; float: left; height: 100%;"></div>
    <div style="width: 30%; background-color: #264de4; float: left; height: 100%;"></div>
    <div style="width: 20%; background-color: #e34c26; float: left; height: 100%;"></div>
  </div>

  <!-- Legenda das linguagens -->
  <ul style="list-style: none; padding: 0; margin: 0;">
    <li style="margin-bottom: 8px;">
      <span style="display: inline-block; width: 10px; height: 10px; background-color: orange; border-radius: 50%; margin-right: 8px;"></span>
      Java - 50%
    </li>
    <li style="margin-bottom: 8px;">
      <span style="display: inline-block; width: 10px; height: 10px; background-color: #264de4; border-radius: 50%; margin-right: 8px;"></span>
      CSS3 - 30%
    </li>
    <li>
      <span style="display: inline-block; width: 10px; height: 10px; background-color: #e34c26; border-radius: 50%; margin-right: 8px;"></span>
      HTML5 - 20%
    </li>
  </ul>
</div>

<!-- Tecnologias em estudo -->
<div style="margin-top: 30px; background-color: #1e1e1e; border: 1px solid #333; border-radius: 10px; padding: 20px; max-width: 500px; color: white;">
  <h3 style="margin-bottom: 10px;">Em estudos</h3>
  <ul style="list-style: none; padding: 0; margin: 0;">
    <li style="display: flex; align-items: center; margin-bottom: 10px;">
      <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/spring/spring-original.svg" alt="Spring" height="24" style="margin-right: 10px;">
      Spring 
    </li>
  </ul>
</div>

<!-- Ícones principais -->
<div style="display: inline_block; margin-top: 20px;">
  <img align="center" alt="Java-pedro" height="30" width="40" src="https://img.shields.io/badge/Java-orange.svg?logo=data:image/png;base64,">
  
  <img align="center" alt="Pedro-CSS" height="30" width="40" src="https://raw.githubusercontent.com/devicons/devicon/master/icons/css3/css3-original.svg">
  
  <img align="center" alt="Pedro-HTML" height="30" width="40" src="https://raw.githubusercontent.com/devicons/devicon/master/icons/html5/html5-original.svg">
</div>
